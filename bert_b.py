...
if __name__ == "__main__":   
    ...
    train = process_B(train_dataset)
    evali = process_B(eval_dataset)
    test = process_B(test_dataset)
    tokenizer = AutoTokenizer.from_pretrained(args.model_name, add_prefix_space=True)
    
    label = ['O'] + [key for key, value in new_label2id.items() if value != 0]
    ...
    main(encoded_ds=encoded_ds, label=label, model_name=model_name, tokenizer=tokenizer, ori_id2label=label2id, ori_label2id=id2label, output_dir=output_dir, save_dir=save_dir)
